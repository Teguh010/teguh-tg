...
export default function DatePickerWithRange({ className = undefined, setStartDate, setEndDate, startDate, endDate, settings = undefined }) {
  ...
  const getValidDateFormat = (format) => {
    ...
    return formatMap[format] || 'yyyy-MM-dd';
  }
  ...
  return (
    <div className={cn("grid gap-2", className)}>
      <Popover>
            ...
            <div className="flex flex-col lg:flex-row">
              {startDate && (
                <div className="flex flex-row items-center">
                  ...
                  >
                    {format(startDate, `${getValidDateFormat(dateFormat)} ${getValidTimeFormat(timeFormat)}`)}
                  </Badge>
                </div>
              )}
              {endDate && (
                <div className="flex flex-row items-center">
                  <span className="lg:pl-2 capitalize">{t('to')}</span>
                  <Separator orientation="vertical" className="mx-2 h-4" />
                  <Badge
                    color="secondary"
                    className="rounded-sm px-1 font-normal"
                  >
                    {format(endDate, `${getValidDateFormat(dateFormat)} ${getValidTimeFormat(timeFormat)}`)}
                  </Badge>
                </div>
              )}
            </div>
        ...
        <PopoverContent className="w-[90%] md:w-auto p-0" align="end">
          <div className="w-full flex flex-row lg:flex-col h-[500px] overflow-y-scroll lg:h-auto lg:overflow-hidden">
            <Calendar
              initialFocus
              mode="range"
            ...
            />
            <div className="flex flex-col lg:flex-row justify-around pb-2 pl-4 lg:pl-0">
              <TimePicker
          ...
          <div className="flex flex-row w-full justify-end gap-1 p-2">
            ...
            <Button
              ...
              color="dark"
              size="xxs"
              onClick={() => (setDate(null), setStartDate(null), setEndDate(null), setStartTime("00:00"), setEndTime("00:00"))}
            >
              {t('reset')}
            </Button>
            <PopoverClose asChild>
